...
WebGL only earns its place when an effect has to **generate** a per-pixel field every frame — procedural noise, grain, a fragment-shader-driven burn front — rather than composite one. Six effects in this pack cross that line: the film burn transition, the filmstock video background, the particulate dissolve, the light leak transition, the bokeh overlay, and the crumpled paper overlay.

### The dual-renderer pattern

The film burn transition, the filmstock video background, the particulate dissolve, the light leak transition, the bokeh overlay, and the crumpled paper overlay all need a genuinely generated per-pixel field to look right, so each ships **two interchangeable renderers behind one factory**, selected with a `renderer: 'auto' | 'webgl' | 'css'` option and reported back on the handle as `activeRenderer`. `'auto'` (the default) prefers WebGL and falls back to the CSS/SVG renderer when WebGL2 is unavailable. For the dissolve, the CSS/SVG + Canvas 2D renderer works over arbitrary live DOM, while the WebGL renderer is a higher-fidelity upgrade reserved for images and video. For the light leak, the two renderers are matched by construction: both read the same staging module for the leak's geometry, so the WebGL path evaluates in closed form what the CSS path stacks as blurred gradient layers. The bokeh goes furthest: both renderers consume one shared disc list, itself derived from highlights detected in the host's own artwork, so they differ only in what a shader can add on top — aperture blades, a fringed rim, and exponential highlight roll-off.
...
| **Bokeh** — defocused highlights placed on the bright points detected in your own picture, each in that point's colour, shimmering as the reader scrolls. | overlay | WebGL, with a CSS gradient fallback renderer (`renderer: 'auto' \| 'webgl' \| 'css'`) | Yes |
| **Ripped Page** — the shot on screen is a printed page: it tears across, the fibres let go in bundles, and the halves hinge apart to reveal the next one. | transition | CSS `clip-path` plus two SVG filters, no WebGL | Yes |
| **Crumpled Paper** — your content printed on a sheet that was screwed up and flattened out again: irregular panels, creases where they meet, fibre tooth, and dirt in the folds. | overlay | WebGL, with a Canvas 2D + SVG lighting fallback renderer (`renderer: 'auto' \| 'webgl' \| 'css'`) | No |
| **Street Light** — one lamp on a mast, standing still while the page walks underneath: whatever is under the pool is lit warm and hard-edged, and the rest of the street stays cold and dark. | overlay | CSS blend-mode layers and gradients, no SVG, no WebGL | Yes |
| **Neon Sign** — your own heading wired to the mains: a hot glass core inside a gas halo, buzzing and dropping out, pooling its light onto whatever sits behind it. | overlay | CSS `text-shadow` and blend-mode gradients, no SVG, no WebGL | No |
| **Wiggly Text** — type that will not sit still: a hand redrawing the same line a few times a second, on any font, any size and any script, with the text left real and selectable. | annotation | CSS `filter` plus four SVG filter primitives, no WebGL | No |
...
**WebGL2 is optional, not required.** The six effects that use it — the film burn transition, the filmstock video background, the particulate dissolve, the light leak transition, the bokeh overlay, and the crumpled paper overlay — treat it as an upgrade over their CSS/SVG (and, for the filmstock and the dissolve, Canvas 2D) renderers. When `createQuadRenderer` cannot get a context, or a shader fails to compile, it logs and returns `null`. All six treat that as a normal outcome, not an error: they fall back to their CSS/SVG renderer, which reproduces the same look without a GPU, and report `activeRenderer === 'css'`. A reader on a machine without WebGL2 sees the same effect, never a hole in the page.
